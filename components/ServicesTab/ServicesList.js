      ...
      <section className="relative w-full px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div
            ...
            style={{
              backgroundImage: "radial-gradient(circle at 30% 50%, rgba(37,99,235,0.25) 0%, transparent 50%), radial-gradient(circle at 70% 50%, rgba(31,41,51,0.4) 0%, transparent 50%)",
            }}
          />
          <Image
          ...
          <div className="relative z-20 h-full flex items-center justify-center px-6 sm:px-8">
            <div className="text-center max-w-4xl">
              ...
              >
                ...
                <span className="text-white/90 text-sm font-medium tracking-wide">Serving Brisbane & Surrounds</span>
              </motion.div>
              <motion.h1
                custom={1}
                variants={heroVariants}
              ...
              >
                PROFESSIONAL <span className="text-accent drop-shadow-lg">TILING</span> SERVICES
              </motion.h1>
              <motion.p
                custom={2}
                variants={heroVariants}
              ...
              >
                {["10+ Years Experience", "Licensed & Insured", "Free Quotes"].map((badge) => (
                  <motion.span
                    key={badge}
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-4">
            <div>
              ...
              <p className="mt-4 text-text-muted text-lg sm:text-xl max-w-2xl leading-relaxed">
                Premium tiling and renovation services tailored for Brisbane homes and businesses.
              </p>
            </div>
            <span className="inline-flex items-center px-4 py-2 rounded-full bg-accent/10 text-accent text-xs font-bold uppercase tracking-widest border border-accent/20">
              Brisbane Wide
            </span>
          </div>
        </motion.div>
          ...
          {servicesData.map((service) => (
            <motion.div key={service.id} variants={itemVariants}>
              <Link href={`/services/${service.slug}`} className="block h-full group">
                <motion.article
                  className="h-full bg-card rounded-3xl overflow-hidden border border-border/60 shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col"
                  whileHover={{ y: -10 }}
                ...
                >
                  <div className="relative h-56 sm:h-60 overflow-hidden">
                    ...
                    <div className="absolute top-5 left-5 z-20">
                      <span className="bg-white/10 backdrop-blur-md text-white text-xs font-medium px-4 py-2 rounded-xl border border-white/20">
                        Brisbane
                      </span>
                    </div>
                    <motion.div
                  ...
                  <div className="p-6 sm:p-8 flex-grow flex flex-col">
                    <div className="flex items-start gap-4 mb-5">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xl sm:text-2xl font-bold text-text-primary group-hover:text-accent transition-colors duration-300 line-clamp-1">
                          {service.title}
                        </h3>
                        <div className="flex items-center gap-2 flex-wrap mt-2">
                          <span className="text-xs font-semibold text-accent bg-accent/10 px-2.5 py-1 rounded-lg">
                            {service.duration}
                          </span>
                          <span className="text-sm text-text-muted">From {service.pricing}</span>
                        </div>
                      </div>
                    </div>

                    <p className="text-text-muted mb-6 line-clamp-3 flex-grow leading-relaxed">
                      {service.shortDesc}
                    </p>
